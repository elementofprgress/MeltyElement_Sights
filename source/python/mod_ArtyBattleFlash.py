...
class Ver:
    modify = '06.03.2018'
    version = 'MeltyElement: Beta v1.0'
    xmlpath = ""
...
class MeltyElement():
    def __init__(self):
        ...
        fp = open(Ver.xmlpath + "/gui/flash/ElementCrosshairSettings.xml")
        s = fp.read()
...
def check():
    ...
    for folder in sb.values():
        ...
        if os.path.exists(filename):
            conp = filename
            Ver.xmlpath = os.getcwd() + folder.asString
            break
    if conp is not None:
        print "--------------------------------------------------------------------------------------"
        print "[MeltyElement]: Loading Element Crosshairs Mod - Arty " + time.strftime("%H:%M:%S")
        print Ver.version + ' Built' + str(Ver.modify)
        print "--------------------------------------------------------------------------------------"
    else:
        print "--------------------------------------------------------------------------------------"
        print "[MeltyElement]: Loading Element Crosshairs Mod - Arty " + time.strftime("%H:%M:%S")
...
#         artyBattleFlash.aimingDistance = '%.0f' % elementAimInfo['aimingDistance']
#         realDispersionAngle = '%.1f' % (elementAimInfo['realDispersionAngle'] * 100)
#         artyBattleFlash.remainingAimingTime = '%.1f' % elementAimInfo['remainingAimingTime'] if artyBattleFlash.aimtime else 9999
#         flightTime = '%.1f' % elementAimInfo['flyTime'] if artyBattleFlash.flighttime else 9999
#         artyBattleFlash.updateBallistics(artyBattleFlash.aimingDistance, flightTime, artyBattleFlash.remainingAimingTime, realDispersionAngle)
#         # TODO
#         reloadingState = ammoCtrl.getGunReloadingState()
#         if BattleReplay.isPlaying() and artyBattleFlash.reloadTime != 0:
#             reloadTime = artyBattleFlash.reloadTime
#             reloadTimer = reloadingState.getTimeLeft()
#             artyBattleFlash.ReloadTime(reloadTimer, gunPitch, reloadTime)
#         else:
#             if reloadingState.isReloading():
#                 reloadTimer = reloadingState._startTime + reloadingState._baseTime - BigWorld.time()
#             else:
#                 reloadTimer = 0
#             artyBattleFlash.ReloadTime(reloadTimer, gunPitch, artyBattleFlash.reloadTime)
#             # self._flashCall('reloadTimer', [reloadTimer, gunPitch, artyBattleFlash.reloadTime])
#     aimMode = BigWorld.player().inputHandler.aim.mode
#     if BigWorld.target() is not None:
#         # noinspection PyBroadException
#         try:
#             curTarget = BigWorld.target()
#             curTargetSpeed = int(round(getSpeed(curTarget) * artyBattleFlash.speedmulti, 0))
#         except:
#             curTargetSpeed = 9999
#         artyBattleFlash.updateVehicleSpeed(curTargetSpeed)
...
#         artyBattleFlash.updateTarget(newDistance, BigWorld.entity(BigWorld.target().id).health)
#         artyBattleFlash.aimingDistance = '%.0f' % newDistance
#         _gun = typeDescr.gun
...
#         artyBattleFlash.setTargetParams(targetReload, targetVisionRadius, newDistance)
#     else:
#         curTargetSpeed = 9999
#         artyBattleFlash.updateVehicleSpeed(curTargetSpeed)
...
#         # self._flashCall('gunView', [gunPitch, v.length])
#     if aimMode == 'arcade' or aimMode == 'sniper' or aimMode == 'strategic' or aimMode == 'postmortem':
#         # self._flashCall('updateBallistics', [_getAdvBallistics()])
#         # self._flashCall('markerDistance', [_getAimDistance()])
#         artyBattleFlash.updateDistance(artyBattleFlash.aimingDistance)
#         # self._flashCall('updateDistance', [aimingDistance])
#         # self._flashCall('updateDistanceMS', [_getAimDistance()])
#     # self._flashCall('aimingEnded', [BigWorld.player().complexSoundNotifications._ComplexSoundNotifications__isAimingEnded])
#     return
...
def new_update():
    ...
    if ownVehicle is not None:
        elementAimInfo = AimingInfo.getMacroData(aimInfo)
        artyBattleFlash.aimingDistance = '%.0f' % elementAimInfo['aimingDistance'] if aimMode != 'strategic' else '%.0f' % BigWorld.camera().position.flatDistTo(BigWorld.player().getOwnVehiclePosition())
        realDispersionAngle = '%.1f' % (elementAimInfo['realDispersionAngle'] * 100)
        ...
        if BattleReplay.isPlaying() and artyBattleFlash.reloadTime != 0:
            ...
            if reloadingState.isReloading():
                ...
                reloadTimer = 0
            artyBattleFlash.ReloadTime(reloadTimer, gunPitch, artyBattleFlash.reloadTime)  # self._flashCall('reloadTimer', [reloadTimer, gunPitch, artyBattleFlash.reloadTime])
    if BigWorld.target() is not None:
        # noinspection PyBroadException
        try:
            curTarget = BigWorld.target()
    ...
    if aimMode == 'arcade':
        artyBattleFlash.component.visible = False
    if aimMode == 'sniper':
        artyBattleFlash.component.visible = False
        curZoom = BigWorld.player().inputHandler.ctrl.camera._SniperCamera__zoom
        zoom2 = 1.047197580337524 / curZoom
        artyBattleFlash.getZoom(zoom2)
    if aimMode == 'strategic':
        # print "update is stratigic!"
        artyBattleFlash.component.visible = True
        height = int(round(BigWorld.camera().position.y - AvatarInputHandler.control_modes.getFocalPoint()[1]))
        artyBattleFlash.getZoom(height)
    if aimMode == 'arcade' or aimMode == 'sniper' or aimMode == 'strategic' or aimMode == 'postmortem':
        artyBattleFlash.updateDistance(artyBattleFlash.aimingDistance)  # self._flashCall('aimingEnded', [BigWorld.player().complexSoundNotifications._ComplexSoundNotifications__isAimingEnded])
...
class ArtyBattleFlash(Flash):
    def __init__(self):
        # Flash.__init__(self, 'ArtyBattleFlash.swf')
        ...
        super(ArtyBattleFlash, self).__init__('ArtyBattleFlash.swf', path=SCALEFORM_SWF_PATH_V3)
        self.reloadTime = 0.0
        self.maxHP = 0.0
        self.aimingDistance = 0.0
        self.remainingAimingTime = 0.0
    ...
    def __del__(self):
        ...
        super(ArtyBattleFlash, self).__del__()
    ...
    def setOnLoad(self, var):
        self.call("ArtyBattleFlash.onLoad", [var])

    def getAimDist(self):
        elementAimInfo = AimingInfo.getMacroData(aimInfo)
        aimingDistance = '%.0f' % elementAimInfo['aimingDistance']
        self.call("ArtyBattleFlash.updateTarget", [aimingDistance])

    def setReloading(self, duration, startTime, isReloading, correction):
        self.call("ArtyBattleFlash.setReloading", [duration, startTime, isReloading, correction])

    # noinspection PyUnusedLocal
    def setReloadingAsPercent(self, percent, isReloading):
        self.call("ArtyBattleFlash.setReloadingAsPercent", [percent])

    def setHealth(self, percent):
        curHP = self.maxHP * percent
        # print "MaxHP: ", self.maxHP, " curHP:", curHP, "percent:", percent
        self.call("ArtyBattleFlash.setHealth", [percent, curHP, self.maxHP])

    def setAmmoStock(self, quantity, quantityInClip, isLow, state, clipReloaded):
        self.call("ArtyBattleFlash.setAmmoStock", [quantity, quantityInClip, isLow, state, clipReloaded])

    # noinspection PyUnusedLocal
    def setClipParams(self, clipCapacity, burst, isAutoloader):
        self.call("ArtyBattleFlash.setClipParams", [clipCapacity, burst])
    ...
    def setTarget(self, isFriend):
        if isFriend:
            ...
            isEnemy = 1
        self.call("ArtyBattleFlash.setTarget", [isEnemy])

    def setTargetParams(self, TargetReload, TargetViewRange, newDist):
        self.call("ArtyBattleFlash.setTargetParams", [TargetReload, TargetViewRange, newDist])

    def updateVehicleSpeed(self, curTargetSpeed):
        self.call("ArtyBattleFlash.updateVehicleSpeed", [curTargetSpeed])

    def updateTarget(self, newDistance, health):
        self.call("ArtyBattleFlash.updateTarget", [newDistance, health])

    def setCenterType(self, centerAlpha, centerType):
        # print "setCenterType:", centerType
        self.call("ArtyBattleFlash.setCenterType", [centerAlpha, centerType])

    def setNetType(self, netAlpha, netType):
        # print "setNetType:", netType
        self.call("ArtyBattleFlash.setNetType", [netAlpha, netType])

    def setReloaderType(self, reloadAlpha, reloadType):
        self.call("ArtyBattleFlash.setReloaderType", [reloadAlpha, reloadType])

    def setConditionType(self, conditionAlpha, conditionType):
        self.call("ArtyBattleFlash.setConditionType", [conditionAlpha, conditionType])

    def setCassetteType(self, cassetteAlpha, cassetteType):
        self.call("ArtyBattleFlash.setCassetteType", [cassetteAlpha, cassetteType])

    def setGunParams(self, gun_Name, shell_Speed, shellGravity, shell_Name):
        # gun_Name, shell_Speed, shellGravity, shell_Name, aimingDistance, flightTime, remainingAimingTime, realDispersionAngle, shellSplash, vehicleName, shellType
        self.call("ArtyBattleFlash.setGunParams", [gun_Name, shell_Speed, shellGravity, shell_Name])

    def updateBallistics(self, aimingDistance, flightTime, remainingAimingTime, realDispersionAngle):
        self.call("ArtyBattleFlash.updateBallistics", [aimingDistance, flightTime, remainingAimingTime, realDispersionAngle])

    def getZoom(self, zoom2):
        self.call("ArtyBattleFlash.getZoomByMeltyMap", [zoom2])

    def onRecreateDevice(self, offsetX, offsetY):
        width, height = GUI.screenResolution()
        self.call("ArtyBattleFlash.onRecreateDevice", [width, height, offsetX, offsetY])

    def clearTarget(self):
        startTime = BigWorld.serverTime()
        self.call("ArtyBattleFlash.clearTarget", [startTime])

    def updateDistance(self, aimingDistance):
        self.call("ArtyBattleFlash.updateDistance", [aimingDistance])

    def ReloadTime(self, reloadTimer, gunPitch, reloadTime):
        self.call("ArtyBattleFlash.reloadTimer", [reloadTimer, gunPitch, reloadTime])
...
offsetYArcade = 458
aimInfo = AimingInfo()
artyBattleFlash = ArtyBattleFlash()
...
# noinspection PyUnusedLocal
@overrideMethod(ArtyAimingSystem, 'getDesiredShotPoint')
def ME_StrategicAimingSystem_getDesiredShotPoint(self, terrainOnlyCheck):
    if BigWorld.player().inputHandler.ctrlModeName != 'strategic':
        # going into "arty" mode is super broken atm, just use normal strategic mode
        # print "aimmode is not strat aimmode is: ", BigWorld.player().inputHandler.ctrlModeName
        return BigWorld.player().gunRotator.markerInfo[0]
    relativeMode = MeltyElement.relativeMode
    result = self.__aimMatrix.translation
    if result is not None:
        if relativeMode:
            if BigWorld.target() is not None:
                target = BigWorld.target()
                targetheight = getVehicleHeight(target)
                heightVector = getVehicleHeightVector(target, targetheight)
                result += heightVector.scale(MeltyElement.relOffset)
    return result


# # noinspection PyUnusedLocal
# @overrideMethod(ArtyAimingSystem, 'getDesiredShotPoint')
# def ME_ArtyAimingSystem_getDesiredShotPoint(self, terrainOnlyCheck):
#     result = self.__aimMatrix.translation
#     if BigWorld.player().inputHandler.ctrlModeName == 'arty':
#         relativeMode = MeltyElement.relativeMode
#         if result is not None:
#             if relativeMode:
#                 if BigWorld.target() is not None:
...
#
#
# @overrideMethod(StrategicAimingSystem, 'getDesiredShotPoint')
# def ME_ArtyAimingSystem_getDesiredShotPoint(self, terrainOnlyCheck):
#     result = AimingSystems.getDesiredShotPoint(self._matrix.translation, Vector3(0, -1, 0), True, True, terrainOnlyCheck)
#     if BigWorld.player().inputHandler.ctrlModeName == 'strategic':
#         relativeMode = MeltyElement.relativeMode
#         if result is not None:
#             if relativeMode:
...
def ME_AP_Vehicle_onLeaveWorld(self):
    global artyBattleFlash
    if self.isPlayerVehicle:
        artyBattleFlash.destroy()
        artyBattleFlash = None


# noinspection PyUnusedLocal
@registerEvent(Vehicle, 'onEnterWorld')
def ME_AP_Vehicle_onEnterWorld(self, prereqs):
    global isNotEvent, artyBattleFlash
    # global artyBattleFlash
    if self.isPlayerVehicle:
        isNotEvent = True
        if artyBattleFlash is None:
            artyBattleFlash = ArtyBattleFlash()
        artyBattleFlash.populate()
        artyBattleFlash.flashObject._x = offsetXArcade
        artyBattleFlash.flashObject._y = offsetYArcade
        artyBattleFlash.setOnLoad(1)
        td = self.typeDescriptor
        artyBattleFlash.maxHP = float(td.maxHealth)
        artyBattleFlash.cameraMode = 'arcade'
        artyBattleFlash.siegeMode = False
...
def CrosshairPanelContainerMeta_as_recreateDeviceS(base, self, offsetX, offsetY):
    ...
    artyBattleFlash.setGunParams(gun_Name, shell_Speed, shellGravity, shell_Name)
    ...
    artyBattleFlash.flashObject._x = offsetX  # (1920 * 0.5) * (1 + offsetX)
    artyBattleFlash.flashObject._y = offsetY  # (0.5 * 1080) * (1 - offsetY)
    # artyBattleFlash.onRecreateDevice(offsetX, offsetY)
...
def CrosshairPanelContainerMeta_as_setReloadingS(self, duration, baseTime, startTime, isReloading):
    artyBattleFlash.reloadTime = baseTime
    artyBattleFlash.setReloading(duration, startTime, isReloading, 0)
    gun_Name, shell_Speed, shellGravity, shell_Name = getGunInfo()
    artyBattleFlash.setGunParams(gun_Name, shell_Speed, shellGravity, shell_Name)
...
def CrosshairPanelContainerMeta_as_setReloadingAsPercent(self, percent, isReloading):
    artyBattleFlash.setReloadingAsPercent(percent, isReloading)
...
def CrosshairPanelContainerMeta_as_setHealthS(self, percent):
    ...
    artyBattleFlash.setHealth(percent)
...
def ElementCrosshaironHealthChanged(self, newHealth, attackerID, attackReasonID):
    if self.isPlayerVehicle:
        newPercent = float(newHealth) / float(artyBattleFlash.maxHP)
        print "onHealthChanged", newHealth, "perc:", newPercent
        artyBattleFlash.setHealth(newPercent)
...
def CrosshairPanelContainerMeta_as_setAmmoStockS(self, quantity, quantityInClip, isLow, clipState, clipReloaded=False):
    artyBattleFlash.setAmmoStock(quantity, quantityInClip, isLow, clipState, clipReloaded)
...
def CrosshairPanelContainerMeta_as_setClipParamsS(self, clipCapacity, burst, isAutoloader=False):
    artyBattleFlash.setClipParams(clipCapacity, burst, isAutoloader)
...
def PlayerAvatar_targetFocus(self, entity):
    if entity in self._PlayerAvatar__vehicles:
        if entity.publicInfo['team'] == 0:
            ...
            isFriend = False
        artyBattleFlash.setTarget(isFriend)
...
def plugins_makeSettingsVO(base, settingsCore, *keys):
    ...
    if isNotEvent:
        for mode in data:
            ...
            artyBattleFlash.setCenterType(100, centerType)
            artyBattleFlash.setNetType(100, netType)
            artyBattleFlash.setReloaderType(100, 0)
            artyBattleFlash.setConditionType(100, 0)
            artyBattleFlash.setCassetteType(100, 0)
...
def ArcadeAimingSystem_enable(self, targetPos, turretYaw=None, gunPitch=None):
    global cameraMode
    cameraMode = 'arcade'
    # artyBattleFlash.flashObject.visible = True
    artyBattleFlash.component.visible = False

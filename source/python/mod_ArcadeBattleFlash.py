...
def check():
    ...
    if conp is not None:
        print "--------------------------------------------------------------------------------------"
        print "[MeltyElement]: Loading Element Crosshairs Mod - Arcade " + time.strftime("%H:%M:%S")
...
def new_update():
    ...
    if BigWorld.target() is not None:
        ...
        try:
            ...
            curTargetSpeed = 9999
        arcadeBattleFlash.updateVehicleSpeed(curTargetSpeed)
        ...
        arcadeBattleFlash.updateTarget(newDistance, BigWorld.entity(BigWorld.target().id).health)
        _gun = typeDescr.gun
        ...
        arcadeBattleFlash.setTargetParams(targetReload, targetVisionRadius, newDistance)
    else:
        curTargetSpeed = 9999
        arcadeBattleFlash.updateVehicleSpeed(curTargetSpeed)  # arcadeBattleFlash.clearTarget()
    aimMode = BigWorld.player().inputHandler.ctrlModeName
    if aimMode == 'arcade':
        arcadeBattleFlash.component.visible = True
    if aimMode == 'sniper':
        arcadeBattleFlash.component.visible = False
    ...
    # if aimMode == 'arcade' or aimMode == 'sniper' or aimMode == 'strategic' or aimMode == 'postmortem':
        # old flash call for aim ended
        # self._flashCall('aimingEnded', [BigWorld.player().complexSoundNotifications._ComplexSoundNotifications__isAimingEnded])
        # self._flashCall('markerDistance', [_getAimDistance()])  #     self._flashCall('updateDistance', [_getAimDistance()])  #     self._flashCall('updateDistanceMS', [_getAimDistance()])
    # return newDistance
...
class ArcadeBattleFlash(Flash):
    def __init__(self):
        # Flash.__init__(self, 'ArcadeBattleFlash.swf')
        ...
        super(ArcadeBattleFlash, self).__init__('ArcadeBattleFlash.swf', path=SCALEFORM_SWF_PATH_V3)
        # 3.6 kmh, 2.23694 mph, 0 meters per sec
        # self.aimtime = True
        # self.flighttime = True
        # self.speedmulti = 3.6
    ...
    def __del__(self):
        ...
        super(ArcadeBattleFlash, self).__del__()
    ...
    def setOnLoad(self, var):
        # print "onload"
        self.call("ArcadeBattleFlash.onLoad", [var])

    def getAimDist(self):
        elementAimInfo = AimingInfo.getMacroData(aimInfo)
        aimingDistance = '%.0f' % elementAimInfo['aimingDistance']
        self.call("ArcadeBattleFlash.updateTarget", [aimingDistance])

    def setReloading(self, duration, startTime, isReloading, correction):
        self.call("ArcadeBattleFlash.setReloading", [duration, startTime, isReloading, correction])

    # noinspection PyUnusedLocal
    def setReloadingAsPercent(self, percent, isReloading):
        self.call("ArcadeBattleFlash.setReloadingAsPercent", [percent])

    def setHealth(self, percent):
        curHP = self.maxHP * percent
        # print "MaxHP: ", self.maxHP, " curHP:", curHP, "percent:", percent
        self.call("ArcadeBattleFlash.setHealth", [percent, curHP, self.maxHP])

    def setAmmoStock(self, quantity, quantityInClip, isLow, state, clipReloaded):
        self.call("ArcadeBattleFlash.setAmmoStock", [quantity, quantityInClip, isLow, state, clipReloaded])

    # noinspection PyUnusedLocal
    def setClipParams(self, clipCapacity, burst, isAutoloader):
        self.call("ArcadeBattleFlash.setClipParams", [clipCapacity, burst])
    ...
    def setTarget(self, isFriend):
        if isFriend:
            ...
            isEnemy = 1
        self.call("ArcadeBattleFlash.setTarget", [isEnemy])

    def setTargetParams(self, TargetReload, TargetViewRange, newDist):
        self.call("ArcadeBattleFlash.setTargetParams", [TargetReload, TargetViewRange, newDist])

    def updateVehicleSpeed(self, curTargetSpeed):
        self.call("ArcadeBattleFlash.updateVehicleSpeed", [curTargetSpeed])

    def updateTarget(self, newDistance, health):
        self.call("ArcadeBattleFlash.updateTarget", [newDistance, health])

    def setCenterType(self, centerAlpha, centerType):
        # print "setCenterType:", centerType
        self.call("ArcadeBattleFlash.setCenterType", [centerAlpha, centerType])

    def setNetType(self, netAlpha, netType):
        # print "setNetType:", netType
        self.call("ArcadeBattleFlash.setNetType", [netAlpha, netType])

    def setReloaderType(self, reloadAlpha, reloadType):
        self.call("ArcadeBattleFlash.setReloaderType", [reloadAlpha, reloadType])

    def setConditionType(self, conditionAlpha, conditionType):
        self.call("ArcadeBattleFlash.setConditionType", [conditionAlpha, conditionType])

    def setCassetteType(self, cassetteAlpha, cassetteType):
        self.call("ArcadeBattleFlash.setCassetteType", [cassetteAlpha, cassetteType])

    def setGunParams(self, gun_Name, shell_Speed, shellGravity, shell_Name):
        self.call("ArcadeBattleFlash.setGunParams", [gun_Name, shell_Speed, shellGravity, shell_Name])

    def updateBallistics(self, aimingDistance, flightTime, remainingAimingTime, realDispersionAngle):
        self.call("ArcadeBattleFlash.updateBallistics", [aimingDistance, flightTime, remainingAimingTime, realDispersionAngle])

    def getZoom(self, zoom2):
        self.call("ArcadeBattleFlash.getZoomByMeltyMap", [zoom2])

    def onRecreateDevice(self, offsetX, offsetY):
        width, height = GUI.screenResolution()
        self.call("ArcadeBattleFlash.onRecreateDevice", [width, height, offsetX, offsetY])

    def clearTarget(self):
        startTime = BigWorld.serverTime()
        self.call("ArcadeBattleFlash.clearTarget", [startTime])

    def toggleIlluminateSight(self, illuminatedToggle, illuminateColor0, illuminateColor1, illuminateColor2):
        self.call("ArcadeBattleFlash.toggleIlluminateSight", [illuminatedToggle, illuminateColor0, illuminateColor1, illuminateColor2])
...
offsetYArcade = 458
aimInfo = AimingInfo()
arcadeBattleFlash = ArcadeBattleFlash()
...
def inject_handle_key_event_arcade(event):
    is_down, key, mods, is_repeat = game.convertKeyEvent(event)
    if hasattr(BigWorld.player(), 'isOnArena') and BigWorld.player().isOnArena and isAlive(BigWorld.player().playerVehicleID) and BigWorld.player().inputHandler.ctrlModeName == 'arcade':
        try:
            if MeltyElement.illuminatedSight:
                ...
                if BigWorld.isKeyDown(MeltyElement.illuminateKey) and is_down and mods == MeltyElement.illuminateModifier:
                    ...
                    if currSight:
                        arcadeMsg = 'Illuminated %s Sight Disabled' % aimMode
                        ...
                        if ctrl is not None:
                            ...
                            battle_page.components['battlePlayerMessages'].as_showRedMessageS(None, arcadeMsg)  # g_appLoader.getDefBattleApp().call('battle.PlayerMessagesPanel.ShowMessage', ['%s illuminateSightOn' % aimMode, 'Illuminated %s Sight Disabled' % aimMode, 'red'])
                    else:
                        arcadeMsg = 'Illuminated %s Sight Enabled' % aimMode
                        ...
                        if ctrl is not None:
                            battle_page = ctrl.containerManager.getContainer(ViewTypes.VIEW).getView()
                            battle_page.components['battlePlayerMessages'].as_showGreenMessageS(None, arcadeMsg)
                    illuminatedToggle = MeltyElement.illumSightVars['illuminatedSightToggle%s' % aimMode]
                    arcadeBattleFlash.toggleIlluminateSight(illuminatedToggle, MeltyElement.illuminateColor[0], MeltyElement.illuminateColor[1], MeltyElement.illuminateColor[2])  # currAim._flashCall('toggleIlluminateSight', [illuminatedToggle, MeltyElement.illuminateColor[0], MeltyElement.illuminateColor[1], MeltyElement.illuminateColor[2]])
            ...
            print('%s inject_handle_key_event' % Ver.version, e)


# noinspection PyUnusedLocal
@registerEvent(Vehicle, 'onLeaveWorld')
def ME_AP_Vehicle_onLeaveWorld(self):
    global arcadeBattleFlash
    if self.isPlayerVehicle:
        arcadeBattleFlash.destroy()
        arcadeBattleFlash = None
        InputHandler.g_instance.onKeyDown -= inject_handle_key_event_arcade
        InputHandler.g_instance.onKeyUp -= inject_handle_key_event_arcade


# noinspection PyUnusedLocal
@registerEvent(Vehicle, 'onEnterWorld')
def ME_AP_Vehicle_onEnterWorld(self, prereqs):
    global isNotEvent, arcadeBattleFlash
    # global arcadeBattleFlash
    if self.isPlayerVehicle:
        isNotEvent = True
        if arcadeBattleFlash is None:
            arcadeBattleFlash = ArcadeBattleFlash()
        arcadeBattleFlash.populate()
        #   Doing something...
        #   Changing properties.
        arcadeBattleFlash.flashObject._x = offsetXArcade
        arcadeBattleFlash.flashObject._y = offsetYArcade
        arcadeBattleFlash.setOnLoad(1)
        # eDebug("Vehicle.onEnterWorld: " + datetime.datetime.now().strftime("%H:%M:%S.%f"))
        td = self.typeDescriptor
        arcadeBattleFlash.maxHP = float(td.maxHealth)
        arcadeBattleFlash.cameraMode = 'arcade'
        arcadeBattleFlash.siegeMode = False
        InputHandler.g_instance.onKeyDown += inject_handle_key_event_arcade
        InputHandler.g_instance.onKeyUp += inject_handle_key_event_arcade
...
def CrosshairPanelContainerMeta_as_recreateDeviceS(base, self, offsetX, offsetY):
    ...
    arcadeBattleFlash.setGunParams(gun_Name, shell_Speed, shellGravity, shell_Name)
    ...
    arcadeBattleFlash.flashObject._x = offsetX  # (1920 * 0.5) * (1 + offsetX)
    arcadeBattleFlash.flashObject._y = offsetY  # (0.5 * 1080) * (1 - offsetY)
    # arcadeBattleFlash.onRecreateDevice(offsetX, offsetY)
...
def CrosshairPanelContainerMeta_as_setReloadingS(self, duration, baseTime, startTime, isReloading):
    arcadeBattleFlash.setReloading(duration, startTime, isReloading, 0)
    gun_Name, shell_Speed, shellGravity, shell_Name = getGunInfo()
    arcadeBattleFlash.setGunParams(gun_Name, shell_Speed, shellGravity, shell_Name)
...
def CrosshairPanelContainerMeta_as_setReloadingAsPercent(self, percent, isReloading):
    arcadeBattleFlash.setReloadingAsPercent(percent, isReloading)
...
def CrosshairPanelContainerMeta_as_setHealthS(self, percent):
    ...
    arcadeBattleFlash.setHealth(percent)
...
def ElementCrosshaironHealthChanged(self, newHealth, attackerID, attackReasonID):
    if self.isPlayerVehicle:
        newPercent = float(newHealth) / float(arcadeBattleFlash.maxHP)
        print "onHealthChanged", newHealth, "perc:", newPercent
        arcadeBattleFlash.setHealth(newPercent)
...
def CrosshairPanelContainerMeta_as_setAmmoStockS(self, quantity, quantityInClip, isLow, clipState, clipReloaded=False):
    arcadeBattleFlash.setAmmoStock(quantity, quantityInClip, isLow, clipState, clipReloaded)
...
def CrosshairPanelContainerMeta_as_setClipParamsS(self, clipCapacity, burst, isAutoloader=False):
    arcadeBattleFlash.setClipParams(clipCapacity, burst, isAutoloader)
...
def PlayerAvatar_targetFocus(self, entity):
    if entity in self._PlayerAvatar__vehicles:
        if entity.publicInfo['team'] == 0:
            ...
            isFriend = False
        arcadeBattleFlash.setTarget(isFriend)
...
def plugins_makeSettingsVO(base, settingsCore, *keys):
    ...
    if isNotEvent:
        for mode in data:
            ...
            arcadeBattleFlash.setCenterType(100, centerType)
            arcadeBattleFlash.setNetType(100, netType)
            arcadeBattleFlash.setReloaderType(100, 0)
            arcadeBattleFlash.setConditionType(100, 0)
            arcadeBattleFlash.setCassetteType(100, 0)

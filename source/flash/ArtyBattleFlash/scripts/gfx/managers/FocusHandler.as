class gfx.managers.FocusHandler
{
    ...
    {
        var _loc2 = this.getPathToFocus();
        if (_loc2.length == 0 || _loc2[0].handleInput == null || _loc2[0].handleInput(event.details, _loc2.slice(1)) != true)
        {
            if (event.details.value == "keyUp")
            {
                return;
            } // end if
            var _loc3 = event.details.navEquivalent;
            if (_loc3 != null)
            {
                if (actualFocus instanceof TextField && Selection.getFocus() == actualFocus && this.textFieldHandleInput(_loc3))
                {
                    return;
                } // end if
            } // end if
